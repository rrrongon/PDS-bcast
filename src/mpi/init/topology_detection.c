...
void* thread1(void *arg){
        ...
        pthread_t t_id = pthread_self(thread1);
        pin_to_core(core, t_id);

        printf("Writing thread working on CPU:%u \n", sched_getcpu());
...
void* thread2(void *arg)
{
        ...
        printf("Reading thread working on CPU:%u \n", sched_getcpu());
...
void pin_to_core(size_t core, pthread_t t)
{
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(core, &cpuset);
