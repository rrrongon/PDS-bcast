    ...
    MPIR_Errflag_t *errflag)
{
	...
	printf("Inside MPIR_Bcast_binomial: 176\n");
        ...
        MPIR_Errflag_t *errflag)
{
	...
	printf("Inside MPIR_SMP_bcast: 1043\n");
    ...
    if ((nbytes < MPIR_CVAR_BCAST_SHORT_MSG_SIZE) || (comm_ptr->local_size < MPIR_CVAR_BCAST_MIN_PROCS))
    {
        ...
        if (comm_ptr->node_comm != NULL)
        {
		printf("Calling Bcast binomail Line: 1108\n");
            MPIR_Bcast_fn_or_override(MPIR_Bcast_topo_aware_hierarchical_MV2, mpi_errno_ret,
    ...
    else /* (nbytes > MPIR_CVAR_BCAST_SHORT_MSG_SIZE) && (comm_ptr->size >= MPIR_CVAR_BCAST_MIN_PROCS) */
    {
		...
		printf("Calling Bcast binomail Line: 1129\n");
                MPIR_Bcast_fn_or_override(MPIR_Bcast_binomial, mpi_errno_ret,
                                          buffer, count, datatype,
        ...
        MPIR_Errflag_t *errflag )
{
    ...
    {
	printf("Calling Bcast binomial at line: 1304\n");
...
int MPIR_Bcast_impl(void *buffer, int count, MPI_Datatype datatype, int root, MPID_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    {
	/*@rubayet*/
	printf("Calling MPIR_Bcast_topo_aware_hierarchical_MV2 from bcast impl in line 1470\n");
	mpi_errno = MPIR_Bcast_topo_aware_hierarchical_MV2(buffer, count, datatype, root, comm_ptr, errflag);
	///////////////
